...
/*!< Tickless timer base address.*/
#define TICKLESS_LPTMR_IRQn LPTMR0_IRQn
/*!< Tickless timer IRQ number. */
/* Task priorities. */
/* clang-format off */
...
int main(void)
{
	/* Define the init structure for the input switch pin */
	    ...
	    #if QUE_MODE == ON
	     adc_queue = xQueueCreate(64, sizeof(uint16_t));
	     // dsp_queue = xQueueCreate(64, 16);
	    #endif
...
/************** Queue mode ON ***********************************************************/
			 ...
			 #if QUE_MODE == ON
			 transfer_DMA_2(adc_queue->pcHead,dsp_buff,64*2); //for queue transfer to array
			 #endif

			/******************Semaphore mutex implementation mutex based semaphore is used to share LED between the tasks.**********************************************************/
			// if( xSemaphore_led != NULL )
			// {
				/* See if the mutex can be obtained. If the mutex is not available wait 10 ticks to see if it becomes free. */
				// if( xSemaphoreTake( xSemaphore_led, 0 ) == pdTRUE )
		...
		else
		{
			...
			count++;
				...
				// PRINTF("RECEIVE STATUS for add itemmmm of adcccc bufferrrrr ISSS %d",receive_status);
				 ...
				 printf("\n \r QUEUE mode on");
			//     uint16_t receive;
				 xQueueSendToBack(adc_queue, &g_Adc16ConversionValue, 0);
			    // xQueueReceive( adc_queue, &receive, portMAX_DELAY );
			    // PRINTF("valueeeeeeee receieved from queueeeeeeeeeeeeeeeeeeee %d",receive);
			 #endif
...
for (uint8_t count2 = 0; count2 < 64;count2++)
{
	...
	sd = sd + pow(((buff_val[count]) - avg), 2);
}
sd=sqrt(sd/64.00);
